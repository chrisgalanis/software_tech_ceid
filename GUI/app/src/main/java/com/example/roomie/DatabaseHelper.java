...
public class DatabaseHelper extends SQLiteOpenHelper {
  ...
  public List<String> getHousePhotos(long houseId) {
    ...
    Cursor c =
        db.query(
            ...
            null);
    while (c.moveToNext()) {
      photos.add(c.getString(c.getColumnIndexOrThrow(COLUMN_HOUSE_PHOTO_URI)));
    }
    c.close();
    return photos;
  }
  ...
  public House getHouseById(long houseId) {
    ...
    return new House(id, ownerId, address, rent, area, floor, latitude, longitude, photoUrls);
  }
  ...
  public List<HouseListing> getAllHouseListings() {
    ...
    while (c.moveToNext()) {
      ...
      if (house != null) {
        listings.add(
            new HouseListing(
                house, ownerName, ownerAvatarUrl, houseAvatarUrl, isApproved // pass the new flag in
                ));
      }
    }
    c.close();
  ...
  /**
   * Inserts a new house‐listing, automatically inserting the House first if needed so that the
   * foreign‐key constraint always passes.
   */
  public long insertHouseListing(HouseListing listing) {
    ...
    try {
      ...
      if (listing.house.id <= 0) {
        ...
        if (newHouseId <= 0) {
          throw new android.database.SQLException("Failed to insert House for listing");
        }
      ...
      listingId = db.insert(TABLE_HOUSE_LISTINGS, null, cvList);
      if (listingId == -1) {
      ...
      Log.e("DatabaseHelper", "Error inserting house listing: " + e.getMessage());
    } finally {
      db.endTransaction();
    }
  ...
  public List<Report> getAllPendingReports() {
    ...
    String[] cols = {COLUMN_REPORT_ID, COLUMN_REPORT_USER_ID, COLUMN_REPORT_TEXT};
    String sel = COLUMN_REPORT_STATUS + "=?";
    String[] args = {"PENDING"};

    Cursor c = db.query(TABLE_REPORTS, cols, sel, args, null, null, COLUMN_REPORT_ID + " DESC");
    while (c.moveToNext()) {
  ...
  public void dismissReport(long reportId) {
    ...
    db.update(TABLE_REPORTS, cv, COLUMN_REPORT_ID + "=?", new String[] {String.valueOf(reportId)});
    db.close();
  }
  ...
  public List<Warning> getWarningsForUser(long userId) {
    ...
    Cursor c = db.query(TABLE_WARNINGS, cols, sel, args, null, null, COLUMN_WARNING_TIME + " DESC");
    while (c.moveToNext()) {
  ...
  public void acknowledgeWarning(long warningId) {
    ...
    db.update(
        TABLE_WARNINGS, cv, COLUMN_WARNING_ID + "=?", new String[] {String.valueOf(warningId)});
    db.close();
  }
}
