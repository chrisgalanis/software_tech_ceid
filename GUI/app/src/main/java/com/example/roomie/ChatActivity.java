...
public class ChatActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (intent != null) {
      ...
      chatPartnerProfileImageRes =
          intent.getIntExtra("PROFILE_IMAGE_RES", R.drawable.roomie_logo); // Default
      // chatPartnerUserId = intent.getStringExtra("USER_ID"); // If you pass a user ID

      Log.d(
          "ChatActivity",
          "onCreate: Received USER_NAME="
              + chatPartnerName
              + ", PROFILE_IMAGE_RES="
              + chatPartnerProfileImageRes);
    } else {
    ...
    ivToolbarProfile =
        findViewById(R.id.ivToolbarProfile); // Ensure this ID exists in activity_chat.xml
    ...
    if (tvToolbarTitle != null && chatPartnerName != null) {
      ...
      Log.e(
          "ChatActivity",
          "onCreate: tvToolbarTitle or chatPartnerName is null after intent processing.");
    ...
    loadDummyMessagesForUser(); // This method should use chatPartnerName/Id to load relevant
    // messages
    chatMessageAdapter =
        new ChatMessageAdapter(
            this, messageList); // Assuming ChatMessageAdapter is for these messages
    ...
    ivInfoButton.setOnClickListener(
        v ->
            Toast.makeText(
                    ChatActivity.this, "Info clicked for " + chatPartnerName, Toast.LENGTH_SHORT)
                .show());
    btnViewProfile.setOnClickListener(
        v ->
            Toast.makeText(
                    ChatActivity.this, "View Profile for " + chatPartnerName, Toast.LENGTH_SHORT)
                .show());
  ...
  private void loadDummyMessagesForUser() {
    ...
    if (chatPartnerName != null && chatPartnerName.equals("Mike M.")) {
      ...
      messageList.add(
          new ChatMessage(
              "currentUser", chatPartnerName, "I dont talk to strangers..", "4:19", true));
      messageList.add(
          new ChatMessage(
              chatPartnerName,
              "currentUser",
